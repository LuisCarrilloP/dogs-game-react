        ...
        <h1 className="text-center font-bold pt-3 pb-10 break-all text-4xl md:text-7xl">{state.currentQuestion.breed}</h1>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 px-5">
            {state.currentQuestion.photos.map((photo, index) => {
              return <div onClick={() => dispatch({type: "guessAttempt", value: index})} key={index} className="rounded-lg h-40 lg:h-80 sm:h-20 md:h-40 bg-cover bg-no-repeat bg-center" style={{backgroundImage: `url(${photo})`}}></div>
            })}
          </div>
        </>
      )}
